...
const newsItems = [
  {
    ...
    description: "La charla abordará una implementación del algoritmo de Laporta para la reducción de integrales multiloop. Se introducirá el rol de estas integrales en procesos de dispersión de partículas, la relación con diagramas de Feynman y el uso de relaciones IBP.",
  ...
  },
  {
    ...
    description: "QState abre sus canales oficiales para compartir investigación, formación, actividades académicas y divulgación científica.",
  },
  {
    date: "2026",
    ...
    speaker: "QState",
    location: "USM",
    description: "El grupo participa en proyectos Fondecyt Regular, Fondecyt Exploración y Anillo Temático, fortaleciendo líneas en sistemas abiertos, control, simulación e inteligencia artificial.",
  },
];
...
const supportItems = [
  {
    name: "ANID Chile",
    role: "Apoyo a la investigación científica",
    description:
      "QState desarrolla investigación vinculada a proyectos financiados por ANID, incluyendo Fondecyt Regular, Fondecyt Exploración y Anillo Temático.",
  },
  {
    name: "CoreDevX",
    role: "Apoyo tecnológico y estratégico",
    description:
      "Empresa chilena que apoya a QState en desarrollo tecnológico, presencia digital e infraestructura para comunicación científica.",
  },
];
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-6 py-4">
        ...
        <nav className="hidden flex-wrap justify-end gap-x-4 gap-y-2 text-base font-medium text-slate-300 xl:flex">
          {navItems.map((item) => (
            <button
              key={item.id}
              type="button"
        ...
        <select
          ...
          className="max-w-[220px] rounded-full border border-white/10 bg-slate-900 px-4 py-2 text-sm text-white outline-none xl:hidden"
  ...
  return (
    <div className="relative mx-auto flex aspect-[1.35/1] w-full max-w-7xl items-center justify-center overflow-hidden rounded-[2rem] border border-cyan-200/20 bg-slate-950/70 shadow-2xl shadow-cyan-950/50 backdrop-blur sm:aspect-[2.2/1] md:rounded-[2.75rem] lg:aspect-[2.9/1]">
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 1000 340">
        ...
        <ellipse cx="500" cy="170" rx="145" ry="88" fill="none" stroke="#67e8f9" strokeOpacity="0.18" />
        <ellipse cx="500" cy="170" rx="230" ry="132" fill="none" stroke="#a78bfa" strokeOpacity="0.16" />
        <ellipse cx="500" cy="170" rx="330" ry="170" fill="none" stroke="#60a5fa" strokeOpacity="0.10" />
      ...
      <div className="mx-auto mt-10 grid max-w-7xl gap-10 lg:mt-12 lg:grid-cols-[0.95fr_1.05fr] lg:gap-12">
        <div>
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            ...
            <button type="button" onClick={() => onNavigate("proyectos")} className="rounded-full border border-white/15 px-6 py-3 text-center font-semibold text-white transition hover:bg-white/10">
              Ver publicaciones
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {researchLines.map((line) => (
            <article key={line.title} className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 shadow-xl shadow-slate-950/30 transition hover:-translate-y-1 hover:border-cyan-200/40 hover:bg-white/[0.07]">
              <div className="mb-6 text-4xl">⚛️</div>
              <h3 className="text-2xl font-semibold text-white">{line.title}</h3>
              <p className="mt-1 text-sm font-medium uppercase tracking-[0.2em] text-cyan-200/70">{line.subtitle}</p>
              <p className="mt-5 leading-8 text-slate-300">{line.description}</p>
            </article>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {projects.map((project) => (
            <article key={project.title} className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 shadow-xl shadow-slate-950/30 transition hover:-translate-y-1 hover:border-cyan-200/40 hover:bg-white/[0.07]">
              <p className="mb-3 text-sm font-medium uppercase tracking-[0.2em] text-cyan-200/70">{project.funding}</p>
              <h3 className="text-2xl font-semibold text-white">{project.title}</h3>
              <div className="mt-6">
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Integrantes asociados</p>
        ...
        <div className="mt-20">
          ...
          <div className="mt-14 grid gap-6 lg:grid-cols-2">
            {publications.map((paper) => (
              <article key={paper.title} className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 shadow-xl shadow-slate-950/30 transition hover:-translate-y-1 hover:border-cyan-200/40 hover:bg-white/[0.07]">
                <div className="mb-4 flex flex-wrap items-center gap-3">
                  <span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-200">{paper.type}</span>
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-300">{paper.topic}</span>
                </div>

                <h3 className="text-2xl font-semibold leading-tight text-white">{paper.title}</h3>
                <p className="mt-4 leading-7 text-slate-300">{paper.authors}</p>
                <p className="mt-3 text-sm font-medium uppercase tracking-[0.16em] text-slate-400">{paper.venue}</p>

                <a href={paper.url} target="_blank" rel="noreferrer" className="mt-6 inline-flex rounded-full border border-cyan-200/30 px-5 py-2 text-sm font-semibold text-cyan-100 transition hover:bg-cyan-200 hover:text-slate-950">
                  Ver publicación
                </a>
              </article>
            ))}
    ...
    <section className="px-4 py-16 sm:px-6 md:py-28">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {newsItems.map((item) => (
            <article key={item.title} className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] shadow-xl shadow-slate-950/30 transition hover:-translate-y-1 hover:border-cyan-200/40 hover:bg-white/[0.07]">
              {item.image && (
                <div className="border-b border-white/10 bg-slate-900">
                  <img src={assetPath(item.image)} alt={item.title} className="h-72 w-full object-cover object-top" />
                </div>
              )}

              <div className="p-7">
                <p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-cyan-200/70">{item.date}</p>
                <h3 className="text-2xl font-semibold text-white">{item.title}</h3>
                <p className="mt-3 font-semibold text-cyan-100">{item.speaker}</p>
  ...
  return (
    <section className="px-4 py-16 sm:px-6 md:py-28">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <p className="mb-3 text-sm font-semibold uppercase tracking-[0.35em] text-cyan-300/80">Vinculación y divulgación</p>
          <h2 className="text-3xl font-semibold tracking-tight text-white md:text-5xl">Acercar la ciencia cuántica a la comunidad.</h2>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {members.map(([name, role], index) => {
            ...
            return (
              <article key={name} className={`rounded-3xl border p-5 transition ${isLeader ? "border-cyan-200/40 bg-cyan-200/10 shadow-xl shadow-cyan-950/30 sm:col-span-2 lg:col-span-3" : "border-white/10 bg-white/[0.035] hover:border-cyan-200/30 hover:bg-white/[0.06]"}`}>
                <p className="font-semibold text-white">
                  {name}
                  {isLeader && <span className="ml-3 rounded-full bg-cyan-200 px-3 py-1 text-xs font-semibold text-slate-950">Líder</span>}
                </p>
  ...
  return (
    <section className="px-4 py-16 sm:px-6 md:py-28">
      <div className="mx-auto max-w-5xl rounded-[2.5rem] border border-cyan-200/20 bg-gradient-to-br from-cyan-300/10 via-white/[0.04] to-violet-300/10 p-8 text-center shadow-2xl shadow-cyan-950/30 md:p-14">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.35em] text-cyan-300/80">Conecta con QState</p>
        <h2 className="text-3xl font-semibold tracking-tight text-white md:text-5xl">Síguenos y participa de nuestra comunidad cuántica.</h2>
        ...
        <div className="mt-10 flex flex-col justify-center gap-4 sm:flex-row">
          <a href="https://www.instagram.com/qstate_usm/" target="_blank" rel="noreferrer" className="rounded-full bg-white px-6 py-3 font-semibold text-slate-950 transition hover:bg-cyan-100">
            Instagram
          </a>
          <a href="mailto:qstate.socials@gmail.com" className="rounded-full border border-white/15 px-6 py-3 font-semibold text-white transition hover:bg-white/10">
            qstate.socials@gmail.com
          </a>
        </div>

        <p className="mt-8 text-sm text-slate-500">Universidad Técnica Federico Santa María · Departamento de Física · Campus San Joaquín</p>
      </div>
    </section>
  );
}
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <footer className="border-t border-white/10 px-4 py-10 sm:px-6">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-5 text-center md:flex-row md:text-left">
          <button type="button" onClick={() => handleNavigate("inicio")} className="flex items-center gap-3 text-left">
            <Logo />
            <div>
              <p className="font-semibold text-white">QState</p>
              <p className="text-sm text-slate-500">Quantum Systems Theory And Technology</p>
            </div>
          </button>
